...
$clientTarget = @"
                <details class="matching-section" open>
...
$propTargetPart1 = "let matchHtml = '<details class=`"matching-section`" style=`"margin-top: 12px; border-top: 1px dashed #cbd5e1; padding-top: 12px;`" open>' +"
$propTargetPart2 = "return matchHtml;"
$propRegex = '(?s)' + [regex]::Escape($propTargetPart1) + '.*?' + [regex]::Escape($propTargetPart2)
...
$content = [regex]::Replace($content, $propRegex, $propReplacement)
...
$content = $content.Replace("    checkAuthAndLoad();", $jsReplacement)
