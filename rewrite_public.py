...
# 2. Remove subCategoryArea and detailSelectArea
html = re.sub(r'<!-- Step 1-2 \(Sub Category\) -->.*?</div>\s*<!-- Step 1-3 \(Detail & Penalty\) -->.*?</div>', '', html, flags=re.DOTALL)

# 3. Remove searchForm and resultArea
html = re.sub(r'<!-- Step 3 -->.*?<div id="resultArea"[^>]*>.*?</div>\s*</div>', '</div>', html, flags=re.DOTALL)

# 4. Remove logout button and requireAuth
html = re.sub(r'// Logout function.*?await requireAuth\(\);', '', html, flags=re.DOTALL)

# 5. Simplify Category Selection and remove DEMO_MODE logic and extra expert logic
js_script = '''    <script type="module">
        ...
        const STAGE_CONFIG = {
            '과태료': [
                ...
                { 
                    name: '과태료 부과 | 이의제기', basis: '질서위반행위규제법', 
                    desc: '서면으로 과태료 부과되며 이에 불복하여 60일내 이의제기를 하게 되면 부과처분은 효력을 상실한다.', tip: '' 
                },
                { 
...
html = re.sub(r'<script type="module">.*?</script>', js_script, html, flags=re.DOTALL)

# Add footer if not present
if '<script src="add_footer.js"></script>' not in html:
    html = html.replace('</body>', '    <script src="add_footer.js"></script>\n</body>')
