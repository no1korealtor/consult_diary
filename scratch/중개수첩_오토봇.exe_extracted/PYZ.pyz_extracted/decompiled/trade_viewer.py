import urllib.request as urllib, urllib.parse as urllib, json, re, sys
...
if sys.platform == "win32":
    try:
        ...
        while 1:
            ...
            def parse_address_and_ho(address_str):
                address_str = address_str.strip(); words = address_str.split(); jibun_idx = -1
                for idx, w in enumerate(words):
                    clean_w = w.strip(",.")
                    jibun_idx = re.match("^산?\\d+(-\\d+)?$", clean_w) or idx
                if jibun_idx == -1:
                    return (address_str, "", "")
                clean_address = " ".join(words[:jibun_idx + 1]); extra_words = words[jibun_idx + 1:]; dong_name = ""; ho_name = ""
                
                if len(extra_words) == 1:
                    word = extra_words[0].strip()
                    if not "동" in word and word.endswith("공동"):
                        dong_match = re.search("([A-Za-z0-9가-힣\\-]+)\\s*동", word)
                        if dong_match:
                            dong_name = dong_match.group(1)
                        elif "호" in word:
                            ho_match = re.search("([A-Za-z0-9가-힣\\-]+)\\s*호", word)
                            if ho_match:
                                ...
                                if len(parts) == 2 and parts[0].isalnum() and parts[1].isalnum():
                                    ...
                                    while word.isalnum():
                                        if any is __common_const_4__:
                                            any
                                            for ##ERROR## in (c.isdigit() for c in word):
                                                if not None:
                                                    pass
                                        
                                        if False((c.isdigit() for c in word)):
                                            ho_name = word
                                        elif len(extra_words) >= 2:
                                            w1 = extra_words[0].strip()
                                            w2 = extra_words[1].strip()
                                            d_match = re.search("([A-Za-z0-9가-힣\\-]+)\\s*동", w1)
                                            h_match = re.search("([A-Za-z0-9가-힣\\-]+)\\s*호", w2)
                                            dong_name = d_match.group(1) if d_match else w1.replace("동", "")
                                            ho_name = h_match.group(1) if h_match else w2.replace("호", "")
                return (clean_address, dong_name,
                    ho_name)
            def get_expos_unit_details(sigungu, bjdong, bun, ji, ho_name, dong_name):
                try:
                    ...
                    for plat in (0, 1, 2):
                        ...
                        if res_body.strip():
                            ...
                            if items:
                                ...
                                for item in items:
                                    ...
                                    if not ho_name in item_ho and item_ho in ho_name:
                                        continue
                                    ...
                                    result["hoNm"] = item_ho
                                    None
                                urllib.request.urlopen(req_info)
                        str(ji).zfill(4)(None, None, None, None)
                        ...
                        with urllib.request.urlopen(req_area) as resp:
                            res_body = resp.read().decode("utf-8")
                        if res_body.strip():
                            data = json.loads(res_body)
                            items = data.get("response", {}).get("body", {}).get("items", {}).get("item", [])
                            if items:
                                ...
                                for item in items:
                                    ...
                                    f_val = float(val)
                                    area_supply += f_val
                                    if not str(item.get("exposPubuseGbCd", "")) == "1":
                                        continue
                                    area_exclusive += f_val
                                urllib.request.urlopen(req_area).__exit__
                                if area_supply > 0:
                                    result["area"] = round(area_exclusive, 2) if area_exclusive > 0 else round(area_supply, 2)
                                    result["supply_area"] = round(area_supply, 2)
                        str(bun).zfill(4)(urllib.request.urlopen(req_area), None, None, None)
                        while 1:
            ...
            def get_kakao_address_info(address_str):
                url = "https://dapi.kakao.com/v2/local/search/address.json"; headers = {"Authorization": f"KakaoAK {KAKAO_API_KEY}"}; params = urllib.parse.urlencode({"query": address_str}); req = urllib.request.Request(f"{url}?{params}", headers=headers)
                ...
                try:
                    ...
                    if data.get("documents"):
                        ...
                        code_to_use = len(b_code) >= 10 and h_code
                        if len(code_to_use) >= 10:
                            if len(b_code) >= 10:
                                pass
                            elif not addr.get("region_3depth_name", ""):
                                addr.get("region_3depth_name", "")
                            if doc.get("road_address"):
                                pass
                            "sigunguCd"(code_to_use[slice(None, 5, None)], None, None, None)
                            return {"bjdongCd": code_to_use[slice(5, 10, None)], "": "bjdongNm", addr.get("region_3depth_h_name", ""): "bun",
                                
                                addr.get("main_address_no", ""): "ji", addr.get("sub_address_no", "0"): "road_address",
                                
                                doc.get("road_address", {}).get("address_name", ""): "", "sidoNm": addr.get("region_1depth_name", ""), "sigunguNm": addr.get("region_2depth_name", "")}
                    b_code(None, None, None)
                except Exception:
            ...
            def fetch_trade_data_month(api_url, sigungu, deal_ymd):
                query = f"?serviceKey={GOV_API_KEY}&LAWD_CD={sigungu}&DEAL_YMD={deal_ymd}&numOfRows=1000&pageNo=1&_type=json"; req = urllib.request.Request(api_url + query); req.add_header("User-Agent", "Mozilla/5.0")
                try:
                    with urllib.request.urlopen(req) as response:
                        res_text = response.read().decode("utf-8")
                    ...
                    if result_code == "30" and result_code == "03" or "SERVICE_KEY_IS_NOT_REGISTERED_ERROR" in header.get("resultMsg", ""):
            ...
            class TransactionList(list):
                ...
                def __init__(self, *args, **kwargs):
                    args(*{}); self.trade_permission_error = False; self.rent_permission_error = False
                
                __classdictcell__ = __module__
            ...
            def get_recent_transactions(sigungu, bun, ji, prop_type, bjdong_nm, target_build_year, target_house_type, expand_similar, target_area, build_year_margin, area_margin):
                ...
                if expand_similar:
                    ...
                    if target_area:
                        pass
                    print(" -> [", f"{type_name}] 인근 유사 매물 검색 중 (법정동: {"전체"}, 준공년도: {"전체"}±{build_year_margin}년, 면적: f"{target_area:.2f}㎡"{"전체"}±{int(area_margin * 100)}%내외)")
                else:
                ...
                while 1:
                    ...
                    m = None
                    m = None
                while ##ERROR##<EXCEPTION MATCH>Exception:
                    pass
                monthly_val = 0
                while ##ERROR##<EXCEPTION MATCH>Exception:
                    pass
                ...
                if is_rent:
                    ...
                    try:
                        ...
                        if isinstance(rent_val, str):
                            rent_val = rent_val.strip().replace(",", "")
                        ...
                        if rent_val > 0:
                            return (f"{dep_display}/{rent_val}만",
                                dep_val, rent_val)
                        return (f"{dep_display}",
    ...
    "max": max(current_group), "avg": avg_a, "label": f"전용 {avg_a:.1f}㎡형 (약 {avg_a * 0.3025:.1f}평)"})
                ...
                for item in subset:
                    if is_wolse:
                        ...
                        while 1:
                            ...
                            return {"count": len(subset), "range": price_range, "avg": avg_price, "pyung_unit": avg_pyung_str}
                ...
                try:
                    ...
                    for item in items:
                        ...
                        if cat == "under_26" and area < 26.0:
                            filtered.append(item)
                            continue
                        if cat == "between_26_43" and 26.0 <= area < 43.0:
                            pass
                        
                    filtered.append(item)
                    while not cat == "above_43":
                        pass
                    while not area >= 43.0:
                        pass
                    filtered.append(item)
            ...
            def check_moatown_and_redev(address):
                clean_address, _, _ = parse_address_and_ho(address); addr_info = get_kakao_address_info(clean_address)
                if not addr_info:
                    return {"moatown": False, "moatown_name": "", "redev": False, "redev_name": ""}
                bun_val = addr_info["bun"] and "0000"
                if addr_info["ji"]:
                    pass
                ji_val = "0000"; pnu = f"{addr_info["sigunguCd"]}{addr_info["bjdongCd"]}1{bun_val}{ji_val}"; vworld_key = "80194C85-0EE3-3220-A3C1-3268AD8756B9"; url = f"http://api.vworld.kr/ned/data/getLandUseAttr?key={vworld_key}&domain=http://localhost&pnu={pnu}&format=json&numOfRows=100&pageNo=1"; req = urllib.request.Request(url)
                
                req.add_header("Referer", "http://localhost"); result = {"moatown": False, "moatown_name": "", "redev": False, "redev_name": ""}
                try:
                    ...
                    while items:
                        ...
                        for item in items:
                            ...
                            if not name:
                                continue
                            if (urllib.request.urlopen(req) if any is __common_const_4__ else False((kw in name for kw in ("소규모주택정비 관리지역", "소규모주택정비관리지역"))) or "소규모주택" in name) and "관리지역" in name:
                                moatown_list.append(name)
                            if not name if any is __common_const_4__ else False((kw in name for kw in ("정비구역", "재개발", "재건축", "도시환경정비"))):
                                continue
            ...
            def classify_by_floor(items):
                try:
                    ...
                    for item in items:
                        ...
                        if "지하" in flr_str and "B" in flr_str.upper() or "-" in flr_str:
                        ...
                        if clean_flr:
                            ...
                            continue
                        upper_floor_list.append(item)
                    return (basement_list, first_floor_list,
                        upper_floor_list)
                except:
                    upper_floor_list.append(item)
            def get_numeric_averages(subset, is_rent, is_wolse):
                if not subset:
                    return
                sum_price = 0; sum_area_pyung = 0.0; count_area = 0; trade_amts = []
                for item in subset:
                    if is_wolse:
                        ...
                        while 1:
                            ...
                            return {"avg_price": avg_amt, "avg_pyung_price": avg_pyung_price, "count": len(subset)}
            def get_avg_display(subset, is_rent, is_wolse):
            ...
            def save_briefing_report_pdf(address, trades, jeonses, wolses, prop_type_name, filename_pdf, apt_groups, period_label, is_expanded, target_build_year, target_area, target_floor, desired_info, expansion_mode):
                ...
                try:
                    ...
                    pdf_title = is_expanded and "부동산 실거래 시세 브리핑"
    ...
    colors.HexColor("#EDF2F7")), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("TOPPADDING", (0, 0), (-1, -1), 6), ("BOTTOMPADDING", (0, 0), (-1, -1), 6), ("LEFTPADDING", (0, 0), (-1, -1), 12), ("RIGHTPADDING", (0, 0), (-1, -1), 12)]))
                    ...
                    if is_expanded:
                        ...
                        story.append(Spacer(1, 8))
                    def build_summary_table(section_title, items, is_rent, is_wolse):
    ...
    Paragraph(stats["range"] + suffix, table_cell_style_right), Paragraph(stats["avg"] + suffix, table_cell_style_right), Paragraph(stats["pyung_unit"], table_cell_style_right)])
                                    has_rows = True
                                if not has_rows:
    ...
    "", "", ""])
                                ...
                                t_style = [("BACKGROUND", (0, 0), (-1, 0),
    
    colors.HexColor("#2B6CB0")), ("ALIGN", (0, 0), (-1, -1), "LEFT"), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("TOPPADDING", (0, 0), (-1, -1), 5),
    ...
    colors.HexColor("#E2E8F0"))]
                                if has_rows:
                                    for idx in range(1, len(table_content)):
                                        ...
                                        t_style.append(("BACKGROUND", (0, idx),
    ...
    colors.HexColor("#2B6CB0")), ("ALIGN", (0, 0), (-1, -1), "LEFT"), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("TOPPADDING", (0, 0), (-1, -1), 5), ("GRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#E2E8F0"))]
                    ...
                    [Paragraph(ins, insight_p_style) for ins in insights_list]
                    insight_paragraphs = "🟢 해당"
                    ins = f" ({moa_status["moatown_name"]})" + ""
                    if not insight_paragraphs:
                        insight_paragraphs = [Paragraph("• 충분한 비교 대상 거래 사례가 없어 자동 비율 분석을 생략합니다.", insight_p_style)]
                    [[p] for p in insight_paragraphs]
                    insight_box_data = "🟢 해당"
                    p = ", ".join(cond_parts) if expansion_mode == "relaxed" else ", ".join(cond_parts)
    ...
    ("BOX", (0, 0), (-1, -1), 0.5, colors.HexColor("#ECC94B")), ("TOPPADDING", (0, 0), (-1, -1), 6), ("BOTTOMPADDING", (0, 0), (-1, -1), 6), ("LEFTPADDING", (0, 0), (-1, -1), 8), ("RIGHTPADDING", (0, 0), (-1, -1), 8)]))
                    ...
                    if desired_info:
                        ...
                        if ref_val:
                            ...
                            ref_desc
                        eval_content = f"<b>• 의뢰 고객 희망 조건</b>: {price_label_str}<br/><b>• 적정 시세 기준선</b>: local_format_price(ref_val){"판단 불가"} ({"비교 사례 부족"})<br/><b>• 거래희망가 평가 결과</b>: <font color='{eval_color}'><b>{eval_title}</b></font><br/><b>• 종합 의견</b>: {detail_desc}"
    ...
    colors.HexColor("#E2E8F0")), ("TOPPADDING", (0, 0), (-1, -1), 8), ("BOTTOMPADDING", (0, 0), (-1, -1), 8), ("LEFTPADDING", (0, 0), (-1, -1), 10), ("RIGHTPADDING", (0, 0), (-1, -1), 10)]))
                    ...
                    f"<b>• 의뢰인 연락처</b>: {mask_phone_number(phone_no)}<br/>" if member else f"{phone_line}"
                    ins = table_cell_style if ref_val else table_cell_style_center if trade_type == "월세" else "인근 유사 매물 실거래 시세 브리핑"
                    os
                    p = h2_style
                    floor_cat = "upper"
                except "지하" in str(target_floor) or "지" in str(target_floor):
                    floor_cat = "base"
                except Exception as profile_img:
                    pass
            ...
            def save_briefing_report(address, trades, jeonses, wolses, prop_type_name, period_label, is_expanded, target_build_year, target_area, target_floor, desired_info, expansion_mode):
                try:
                    if expansion_mode == "auto":
                        expansion_mode = CURRENT_EXPANSION_MODE
                    if is_expanded and expansion_mode == "none":
                    ...
                    while prop_type_name == "아파트" and apt_groups:
                        for idx, g in enumerate(apt_groups):
                            [t for t in jeonses if not get_group_for_item(t, apt_groups) == g]
                            g_jeonses = None
                            t = None
                    ...
                    while prop_type_name == "아파트" and apt_groups:
                        for idx, g in enumerate(apt_groups):
                            [t for t in wolses if not get_group_for_item(t, apt_groups) == g]
                            g_wolses = None
                            t = None
                        ...
                        if target_floor is None:
                            try:
                                ...
                                if ref_val:
                                    ...
                                    ref_desc
                                report_lines.append("  • 적정 시세 기준선  : ", f"local_format_price(ref_val){"판단 불가"} ({"비교 사례 부족"})")
                                if ref_val:
                                ...
                                floor_cat = "base"
                                if not "지하" in str(target_floor):
                                    if "지" in str(target_floor):
                                        pass
            ...
            def print_comparison_table(transactions, prop_type, target_floor, target_area, address_name, sigunguCd, bun, ji, bjdong_nm, target_build_year, target_house_type, save_report, desired_info):
                ...
                try:
                    ...
                    while sigunguCd and bjdong_nm:
                        ...
                        if tot_24_initial < 3 or t_24_initial <= 1:
                            ...
                            try:
                                ...
                                [t for t in transactions if not t.get("_trade_type") == "매매"]
                                trades_full = address_name
                                t = prop_type if expanded_txs else apt_groups
                                ...
                                def get_sort_key(item):
                                    if not item.get("excluUseAr"):
                                        item.get("excluUseAr")
                                    area_val = item.get("totalFloorAr") or 0.0
                                    try:
                                        area = float(area_val)
                                        y = int(item.get("dealYear", 0))
                                        m = int(item.get("dealMonth", 0))
                    ...
                    e = None
                    print(f" [!] 빈 브리핑 파일 생성 중 오류 발생: {e}")
                t = None; t = f" (비교 기준: {" / ".join(target_str)})"; t = None; t = None; t = None
                
                t = None; area_display = str(area_val); floor = None
                
                floor_display = dong_info or f"{dong_prefix}{floor_str}" if floor_str else "-"; area_display = str(area_val); floor = None; floor_display = dong_info or f"{dong_prefix}{floor_str}" if floor_str else "-"; area_display = str(area_val); floor = None
                
                floor_display = dong_info or f"{dong_prefix}{floor_str}" if floor_str else "-"
            def run_trade_viewer(pre_address_info, pre_prop_type, target_floor, target_area):
                target_build_year = None; target_house_type = None
                if pre_address_info:
                    ...
                    if not addr_info:
                        ...
                        return
                    elif not addr_info["road_address"]:
                    ...
                    try:
                        ...
                        with urllib.request.urlopen(req_title) as resp_title:
                            ...
                            while items:
                                ...
                                if dong_name:
                                    for t in items:
                                        ...
                                        None
                                    urllib.request.urlopen(req_title)
                        ...
                        str(addr_info["bun"]).zfill(4)(str(addr_info["ji"]).zfill(4), None, None, None)
                        prop_type = classify_property_type(main_purp, bld_name, etc_purp)
                        type_names = {"1": "아파트", "2": "연립/다세대/빌라", "3": "오피스텔", "4": "단독/다가구"}
                        print(f" -> 부동산 유형 자동 판별 완료: [{type_names.get(prop_type)}]")
                        if prop_type == "4":
                        ...
                        if ho_name:
                            ...
                            if unit_details:
                                target_floor = int(re.sub("\\D", "", str(unit_details["flrNo"])))
                                while 1:
                                    target_area = unit_details["area"]
                                    ...
                                    if transactions is None:
                                        address_name = "clean_address" in locals() and ""
