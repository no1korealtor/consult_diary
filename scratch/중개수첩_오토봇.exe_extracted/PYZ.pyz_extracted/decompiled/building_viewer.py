        ...
        if os.path.exists(p):
            try:
                ...
                if isinstance(data, dict):
                    ...
                    open(p, "r", encoding="utf-8").__exit__(None, None, None)
                    return profile
...
def format_member_name(name):
    if not name:
        return "공인중개사"
    while any is __common_const_4__:
        any
        for ##ERROR## in (suffix in name for suffix in ("공인중개사", "사무소", "부동산", "컨설팅", "대표")):
            if not None:
                pass
            name
        break
    if False((suffix in name for suffix in ("공인중개사", "사무소", "부동산", "컨설팅", "대표"))):
        return name
...
if sys.platform == "win32":
    try:
        ...
        while 1:
            ...
            def get_kakao_address_info(address_str):
                url = "https://dapi.kakao.com/v2/local/search/address.json"; headers = {"Authorization": f"KakaoAK {KAKAO_API_KEY}"}; params = {"query": address_str}
                try:
                    ...
                    if data.get("documents"):
                        ...
                        if len(b_code) >= 10:
                            if doc.get("road_address"):
                                return {"sigunguCd": b_code[slice(None, 5, None)], "bjdongCd": b_code[slice(5, 10, None)],
                                    
                                    "bjdongNm": addr.get("region_3depth_name", ""), "bun": addr.get("main_address_no", ""),
                                    
                                    "ji": addr.get("sub_address_no", "0"), "road_address": doc.get("road_address", {}).get("address_name", "")}
                            return {None: None, None: None, None: None, None: None, None: None, None: ""}
                    return
                except:
                    pass
            def get_vworld_land_info(vworld_key, pnu):
                url = f"http://api.vworld.kr/ned/data/getLandCharacteristics?key={vworld_key}&domain=http://localhost&pnu={pnu}&format=json&numOfRows=10&pageNo=1"; req = urllib.request.Request(url); req.add_header("Referer", "http://localhost")
                try:
                    ...
                    if items:
                        ...
                        item = items_sorted[0]
                        urllib.request.urlopen(req).__exit__(None, None, None)
                        return {"area": item.get("lndpclAr", ""), "lndcgr": item.get("lndcgrCodeNm", ""),
            ...
            def get_vworld_land_price(vworld_key, pnu):
                url = f"http://api.vworld.kr/ned/data/getLandCharacteristics?key={vworld_key}&domain=http://localhost&pnu={pnu}&format=json&numOfRows=100&pageNo=1"; req = urllib.request.Request(url); req.add_header("Referer", "http://localhost")
                try:
                    with urllib.request.urlopen(req) as resp:
                        data = json.loads(resp.read().decode("utf-8"))
                        items = data.get("landCharacteristicss", {}).get("field", [])
            ...
            def get_vworld_individual_house_price(vworld_key, pnu):
                url = f"http://api.vworld.kr/ned/data/getIndvdHousingPriceAttr?key={vworld_key}&domain=http://localhost&pnu={pnu}&format=json&numOfRows=100&pageNo=1"; req = urllib.request.Request(url); req.add_header("Referer", "http://localhost")
                try:
                    with urllib.request.urlopen(req) as resp:
                        data = json.loads(resp.read().decode("utf-8"))
            ...
            def get_vworld_apartment_house_price(vworld_key, pnu, dong_name, ho_name):
                url = f"http://api.vworld.kr/ned/data/getApartHousingPriceAttr?key={vworld_key}&domain=http://localhost&pnu={pnu}&format=json&numOfRows=1000&pageNo=1"; req = urllib.request.Request(url); req.add_header("Referer", "http://localhost")
                try:
                    with urllib.request.urlopen(req) as resp:
                        data = json.loads(resp.read().decode("utf-8"))
                    ...
                    if items:
                        def match_part(target, item):
                            if not target:
                                ...
                                return False
                            t_str = str(target).strip().lower(); i_str = str(item).strip().lower()
                            if t_str == i_str:
                                return True
                            t_clean = t_str.replace("호", "").replace("동", ""); i_clean = i_str.replace("호", "").replace("동", "")
                            if t_clean == i_clean:
                                return True
                        ...
                        for item in items:
                            ...
                            if ho_name:
                                ...
                                continue
                            matched_items.append(item)
                        urllib.request.urlopen(req).__exit__
                        while matched_items:
                            ...
                            for item in items_sorted:
                                ...
                                if not year_val:
                                    pass
                                None
                                urllib.request.urlopen(req)(None, None, None)
                            return {"year": year_val, "price": int(price_val)}
                    ##ERROR##(None, None, None)
                except:
                    pass
            ...
            def get_vworld_land_use_info(vworld_key, pnu, sigungu_cd):
                url = f"http://api.vworld.kr/ned/data/getLandUseAttr?key={vworld_key}&domain=http://localhost&pnu={pnu}&format=json&numOfRows=100&pageNo=1"; req = urllib.request.Request(url); req.add_header("Referer", "http://localhost"); result = {"moatown": False, "moatown_name": "", "redev": False, "redev_name": "", "permit": False, "permit_name": "", "regulated": False, "regulated_name": ""}; regulated_districts = {"11680": "강남구 (투기과열지구/조정대상지역)", "11650": "서초구 (투기과열지구/조정대상지역)", "11710": "송파구 (투기과열지구/조정대상지역)", "11170": "용산구 (투기과열지구/조정대상지역)"}
                ...
                try:
                    ...
                    while items:
                        ...
                        for item in items:
                            ...
                            if not name:
                                continue
                            if urllib.request.urlopen(req) if any is __common_const_4__ else False((kw in name for kw in ("소규모주택정비 관리지역", "소규모주택정비관리지역"))):
                                moatown_list.append(name)
                            elif "소규모주택" in name and "관리지역" in name:
                                moatown_list.append(name)
                            if any is __common_const_4__:
                                any
                                for ##ERROR## in (kw in name for kw in ("정비구역", "재개발", "재건축", "도시환경정비")):
                                    if not None:
                                        pass
                                    name
                            
                            if False((kw in name for kw in ("정비구역", "재개발", "재건축", "도시환경정비"))):
                                redev_list.append(name)
                            if not "토지거래계약에관한허가구역" in name and "토지거래허가" in name:
                                continue
            ...
            def get_building_title_info(sigungu, bjdong, bun, ji):
                bun_str = bun and "0000"; ji_str = ji and "0000"; url = "http://apis.data.go.kr/1613000/BldRgstHubService/getBrTitleInfo"; query = f"?serviceKey={GOV_API_KEY}&sigunguCd={sigungu}&bjdongCd={bjdong}&platGbCd=0&bun={bun_str}&ji={ji_str}&numOfRows=10&pageNo=1&_type=json"
                try:
                    ...
                    if not res_text.strip():
                        return urllib.request.urlopen(req).__exit__
                    json_data = json.loads(res_text)
                    items = json_data.get("response", {}).get("body", {}).get("items", {}).get("item", [])
                    if not items:
                        str(ji).zfill(4)(urllib.request.urlopen(req), None, None, None)
                        return
                    elif isinstance(items, dict):
                        str(bun).zfill(4)(None, None, None)
                        return [items]
                    ##ERROR##(None, None, None)
                    return items
                except:
            ...
            def get_expos_info_list(sigungu, bjdong, bun, ji):
                bun_str = bun and "0000"; ji_str = ji and "0000"; url = "http://apis.data.go.kr/1613000/BldRgstHubService/getBrExposInfo"; query = f"?serviceKey={GOV_API_KEY}&sigunguCd={sigungu}&bjdongCd={bjdong}&platGbCd=0&bun={bun_str}&ji={ji_str}&numOfRows=100&pageNo=1&_type=json"
                try:
                    req = urllib.request.Request(url + query)
                    req.add_header("User-Agent", "Mozilla/5.0")
                    req.add_header("Accept", "application/json, text/plain, */*")
            ...
            def get_building_floor_info(sigungu, bjdong, bun, ji):
                bun_str = bun and "0000"; ji_str = ji and "0000"; url = "http://apis.data.go.kr/1613000/BldRgstHubService/getBrFlrOulnInfo"; query = f"?serviceKey={GOV_API_KEY}&sigunguCd={sigungu}&bjdongCd={bjdong}&platGbCd=0&bun={bun_str}&ji={ji_str}&numOfRows=100&pageNo=1&_type=json"
                try:
                    req = urllib.request.Request(url + query)
                    req.add_header("User-Agent", "Mozilla/5.0")
                    req.add_header("Accept", "application/json, text/plain, */*")
    ...
    Paragraph(bld_nm_str, value_style)],
                    ...
                    if bld_data["is_jibbap"]:
                        pass
                    bld_info_data = [[Paragraph, "집합건축물"("일반건축물", value_style), Paragraph("<b>대지면적</b>", label_style), Paragraph(f"{bld_data["plat_area"]} ㎡", value_style)], [Paragraph("<b>주용도</b>", label_style), Paragraph(bld_data["main_purp"], value_style), Paragraph("<b>건물구조</b>", label_style),
    
    Paragraph(bld_data["structure"], value_style)],
                        
                        [Paragraph("<b>건축면적</b>", label_style), Paragraph(f"{bld_data["arch_area"]} ㎡", value_style), Paragraph("<b>연면적</b>", label_style), Paragraph(f"{bld_data["tot_area"]} ㎡ (용적률산정: {bld_data["vl_rat_tot_area"]} ㎡)", value_style)],
                        
                        [Paragraph("<b>층수</b>", label_style), Paragraph(f"지상 {bld_data["grnd_cnt"]}층 / 지하 {bld_data["ugrnd_cnt"]}층", value_style), Paragraph("<b>세대/가구수</b>", label_style),
    
    Paragraph(f"공동주택 {bld_data["hhld"]}세대 / 다가구 {bld_data["fmly"]}가구", value_style)], [Paragraph("<b>총 주차대수</b>", label_style), Paragraph(f"{bld_data["parking"]} 대", value_style), Paragraph("<b>사용승인일</b>", label_style),
    
    Paragraph(bld_data["apr_day"], value_style)], [Paragraph("<b>위반 여부</b>", label_style), Paragraph(bld_data["viol_str"], value_style), Paragraph("<b>엘리베이터</b>", label_style), Paragraph(elvt_pdf_val, value_style)]]
                    if bld_data.get("room_count_label"):
                        bld_info_data.append([Paragraph("<b>방 개수</b>", label_style), Paragraph(bld_data["room_count_label"], value_style), Paragraph("", label_style),
    ...
    Paragraph("", value_style)])
                    ...
                    if not bld_data["is_jibbap"]:
                        pass
                    land_price_data = bld_data["is_jibbap"] or [Paragraph(land_price_str, value_style), [Paragraph, "<b>개별주택가격</b>"("", label_style), Paragraph, house_price_str("", value_style)]]
    ...
    (0, 0), (-1, -1), 0.5, colors.HexColor("#EDF2F7")), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("LEFTPADDING", (0, 0), (-1, -1), 8), ("RIGHTPADDING", (0, 0), (-1, -1), 8)]))
                    ...
                    if reg["moatown"]:
                        ...
                        if reg["regulated_name"]:
                            pass
                    reg_table_data = ["⚠️ 지정됨"(f" ({reg["permit_name"]})" + "", "⚪ 미지정", value_style), [Paragraph("<b>조정대상지역</b>", label_style), Paragraph, ##ERROR##,"⚠️ 규제지역"(f" ({reg["regulated_name"]})" + "", "⚪ 비규제지역", value_style)]]
    ...
    ("RIGHTPADDING", (0, 0), (-1, -1), 8)]))
                    ...
                    if bld_data.get("ho_details"):
                        ...
                        if hd.get("area"):
                            pass
                        pyung_area = "정보없음"
                        supply_area = hd.get("supply_area") and "정보없음"
                        land_share_str = "정보없음"
                        ...
                        ho_table_data = [[Paragraph("<b>해당 층수</b>", label_style), Paragraph(f"{hd["flr_no"]}층", value_style), Paragraph("<b>위반 여부</b>", label_style), Paragraph(viol_status, value_style)], [Paragraph("<b>전용면적</b>", label_style), Paragraph(pyung_area, value_style), Paragraph("<b>공급면적</b>", label_style),
    
    Paragraph(supply_area, value_style)],
                            
                            [Paragraph("<b>대지지분</b>", label_style),
    
    Paragraph(land_share_str, value_style), Paragraph("<b>공동주택가격</b>", label_style), Paragraph(apt_price_str, value_style)]]
    ...
    colors.HexColor("#E2E8F0")), ("INNERGRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#EDF2F7")), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("LEFTPADDING", (0, 0), (-1, -1), 8), ("RIGHTPADDING", (0, 0), (-1, -1), 8)]))
                                        ...
                                        if os.path.exists("office_info.txt"):
                                            try:
                                                with open("office_info.txt", "r", encoding="utf-8") as f:
                                                    ...
                                                    if content:
                                                        office_info = content
                                                open("office_info.txt", "r", encoding="utf-8").__exit__(None, None, None)
                                            except:
            ...
            def save_building_report(address, bld_data):
                ...
                [c for c in address if not c in (" ", "-", "_")]
                c = None; safe_addr = ##ERROR##("".join).strip(); filename_txt = f"건축물대장/건축물대장_{safe_addr.replace(" ", "_")}.txt"; filename_pdf = f"건축물대장/건축물대장_{safe_addr.replace(" ", "_")}.pdf"
                ...
                if bld_data["is_jibbap"]:
                    pass
                lines.append("  • 건물 유형: ", f"집합건축물 (공동주택/아파트/오피스텔 등){"일반건축물 (단독/다가구/상가주택 등)"}")
                
                masked_bld_nm = bld_data["bld_nm"] and "정보없음"
                if bld_data["dong_nm"]:
                    pass
                lines.append("  • 건물명/동: ", f"{masked_bld_nm} / bld_data["dong_nm"] + "동"{"정보없음"}"); lines.append(f"  • 대지면적 : {bld_data["plat_area"]} ㎡")
                if not bld_data["lndcgr"]:
                    bld_data["lndcgr"]
                lines.append(f"  • 지목     : {"정보없음"}")
                
                if not bld_data["use_zone"]:
                    bld_data["use_zone"]
                lines.append(f"  • 용도지역 : {"정보없음"}"); land_price_str = "정보없음"
            ...
            def run_building_viewer():
                try:
                    print("\n============================================================")
                    print("      부동산 통합 조회 서비스 (대장 정보 + 실거래가 비교)      ")
                    print("============================================================")
                    print("  ※ [안내] 본 프로그램의 시장 분석 및 보고서는 중개업무 참고용이며,")
                    print("            법적 효력을 가지지 않습니다.")
                    print("  ------------------------------------------------------------")
                    print("  조회할 주소를 입력하시면 건축물대장(표제부/전유부), 대지지분,")
                    print("  그리고 해당 지번의 최근 실거래가(매매/임대차)를 통합 조회하여 비교합니다.")
                    print("  (종료하려면 'q' 또는 엔터를 입력하세요)")
                    print("------------------------------------------------------------")
                    address = input("\n[입력] 조회할 부동산 주소 (예: 성산동 138 402호): ").strip()
                    if address or address.lower() == "q":
                        return
                    ...
                    if not addr_info["road_address"]:
                        addr_info["road_address"]
                    print(f" -> 주소 확인: {clean_address} ({"도로명 없음"})")
                    print(" -> 건축물 표제부 조회 중...")
                    ...
                    while dong_name:
                        for t in titles:
                            selected_title = match_dong(dong_name, t.get("dongNm", "")) or t
                            None
                    ...
                    if reg_gb_cd:
                        if not reg_gb_cd == "2":
                            reg_gb_cd == "2"
                        is_jibbap = reg_gb_nm == "집합"
                    else:
                        is_jibbap = prop_type in ("1", "2", "3")
                    bun_val = addr_info["bun"] and "0000"
                    if addr_info["ji"]:
                        pass
                    ji_val = "0000"
                    ...
                    if ho_name:
                        try:
                            ...
                            hhld = int(selected_title.get("hhldCnt", 0))
                            fmly = int(selected_title.get("fmlyCnt", 0))
                            ...
                            for p_type in ("indrAutoUtcnt", "indrMechUtcnt", "oudrAutoUtcnt", "oudrMechUtcnt"):
                                ...
                                if not val:
                                    continue
                                parking += int(val)
                            f" [{bld_nm}]" if is_jibbap else f" {dong_nm}동"
                            print(f"  • 총 주차대수: {parking} 대")
                            apr = str(selected_title.get("useAprDay", "")).strip()
                            if apr and len(apr) == 8:
                                apr_str = f"{apr[slice(None, 4, None)]}년 {apr[slice(4, 6, None)]}월 {apr[slice(6, 8, None)]}일"
                            elif apr:
                                pass
                            apr_str = "정보없음"
                            ...
                            if apr and len(apr) >= 4:
                                target_build_year = int(apr[slice(None, 4, None)])
                            ...
                            viol_str = viol == "Y" and "정상 (위반 없음)"
                            print(f"  • 위반 여부 : {viol_str}")
                            if not selected_title.get("rideUseElvtCnt", 0):
                                selected_title.get("rideUseElvtCnt", 0)
                            ride_elvt = int(0)
                            if not selected_title.get("emgenUseElvtCnt", 0):
                                selected_title.get("emgenUseElvtCnt", 0)
                            emgen_elvt = int(0)
                            total_elvt = ride_elvt + emgen_elvt
                            if not selected_title.get("grndFlrCnt", 0):
                                selected_title.get("grndFlrCnt", 0)
                            grnd_cnt_int = int(0)
                            if total_elvt > 0:
                            ...
                            if is_jibbap:
                                ...
                                if expos_list:
                                    ...
                                    if dong_name and len(distinct_dongs) > 1:
                                        ...
                                        for item in expos_list:
                                            ...
                                            if not dong_name and match_dong(dong_name, item_dong):
                                                continue
                                            ...
                                            filtered_count += 1
                                        print(f"  • 조정대상지역 : ⚠️ 규제지역 ({reg_info["regulated_name"]})") if reg_info["regulated"] else str(addr_info["bun"]).zfill(4) if reg_info["permit"] else str(addr_info["ji"]).zfill(4) if reg_info["redev"] else apr if reg_info["moatown"] else "⚠️ 위반건축물 지정!!" if unit_info else "⚠️ 위반건축물 지정!!"
                                        print("--------------------------------------------------")
                                        dong_desc = dong_name and ""
                                ...
                                if floors:
                                    ...
                                    def get_floor_sort_key(f):
                                        flr_no_str = str(f.get("flrNo", "0")).strip(); flr_gb = f.get("flrGbCdNm", "") or ""
                                        try:
                                ...
                                url_lda = f"http://api.vworld.kr/ned/data/ldaregList?key={vworld_key}&domain=http://localhost&pnu={pnu}&format=json&numOfRows=100&pageNo=1"
                                req_lda = urllib.request.Request(url_lda)
                                req_lda.add_header("Referer", "http://localhost")
                                with urllib.request.urlopen(req_lda) as resp_lda:
                                    lda_data = json.loads(resp_lda.read().decode("utf-8"))
                                    items = lda_data.get("ldaregVOList", {}).get("ldaregVOList", [])
                                urllib.request.urlopen(req_lda) if items else urllib.request.urlopen(req_lda).__exit__(None, None, None)
                            print("\n==================================================")
                            ...
                            if not is_jibbap and ho_name:
                                ...
                                if ho_input:
                                    ...
                                    if unit_info:
                                        ...
                                        viol_status = unit_info["violBldYn"] == "Y" and "정상 (위반 없음)"
                                        ...
                                        print("--------------------------------------------------")
                                        target_floor = int(re.sub("\\D", "", str(unit_info["flrNo"])))
                                    else:
                            ...
                            if is_jibbap and trade_type in ("전세", "월세"):
                                ...
                                cleaned_floor = floor_input.replace("층", "").strip()
                            ...
                            emgen_elvt = int(0)
                            total_elvt = ride_elvt + emgen_elvt
                            if not addr_info.get("road_address"):
                                addr_info.get("road_address")
                            if land_info:
                                pass
                            elif land_info:
                                pass
                            elif land_price_info:
                                pass
                            elif land_price_info:
                                pass
                            elif is_jibbap and indiv_house_price:
                                pass
                            elif is_jibbap and indiv_house_price:
                                pass
                            elif is_jibbap and ho_name and unit_info:
                                if is_jibbap and ho_name and unit_info:
                                    pass
                                elif is_jibbap and ho_name and unit_info:
                                    pass
                                elif is_jibbap and ho_name and unit_info:
                                    pass
                                elif is_jibbap and ho_name and land_share:
                                    pass
                                elif is_jibbap and ho_name and unit_info:
                                    pass
                                elif is_jibbap and ho_name and apt_house_price:
                                    pass
                                elif is_jibbap and ho_name and apt_house_price:
                                    pass
                            elif is_jibbap:
                                ...
                                pass
                            elif is_jibbap and distinct_dongs:
                                pass
                            elif is_jibbap and sorted_floors:
                                [{f"{""}": f" ["flrNo"]{""}".strip(),
    
    "main_purp": f.get("mainPurpsCdNm", "") or "정보없음", "structure": f.get("strctCdNm", "") or "정보없음", "area": f.get("area", "")} for f in sorted_floors if not f.get("flrNo", "")]
                                f = None
                            while not is_jibbap and items:
                                [{"rate": item.get("ldaQotaRate", ""), "ho": item.get("buldHoNm", "").strip()} for item in items]
                                item = {"flr_desc": None}
                                break
                            bld_data = room_label and "room_count_label"
                            ...
                            [c for c in target_addr_str if not c in (" ", "-", "_")]
                            c = None
                            safe_addr_for_open = {floor_map: None}({list(distinct_dongs): None,
    
    "land_shares_raw": None}, "".join).strip().replace(" ", "_")
                            bld_pdf_path = os.path.abspath(f"종합분석보고서/건축물대장_{safe_addr_for_open}.pdf")
                            brief_pdf_path = os.path.abspath(f"종합분석보고서/시세브리핑_{safe_addr_for_open}.pdf")
                            opened_any = False
                            if sys.platform == "win32":
                            ...
                            if upload_choice == "y":
                                ...
                                if clean_address:
                                    ...
                                    if len(parts) >= 2:
                                        sigungu_name = " ".join(parts[slice(None, 2, None)])
                                bunji_val = addr_info["bun"]
                                if addr_info.get("ji") and addr_info["ji"] != "0":
                                    bunji_val += "-" + addr_info["ji"]
                                preset_data = {"trade_type": trade_type, "price": target_price, "monthly_rent": target_monthly, "phone_number": client_phone, "sigunguCd": addr_info["sigunguCd"], "bjdongCd": addr_info["bjdongCd"],
                                    
                                    "sigungu_name": sigungu_name, "bjdong_name": addr_info.get("bjdongNm", ""),
                                    
                                    "bunji_val": bunji_val, "ho_name": ho_name, "dong_name": dong_nm}
                            ...
                            {{unit_info.get("supply_area"): None, "land_share": land_share,
    None: "viol_yn",
    unit_info.get("violBldYn"): None, "apt_price_year": apt_house_price.get("year"),
    None: "apt_price",
    apt_house_price.get("price"): None}: None}
                            plat_area_float = 0.0
                except:
                    pass
                grnd_cnt_int = 0; print(f"  • 대지지분  : {land_share} ㎡")
                
                target_floor = None; sorted_floors = sorted(floor_map.keys()); sorted_floors = floors; area_str = f"{area_val} ㎡"
                if not __exception__({{unit_info.get("supply_area"): None, "land_share": land_share,
    None: "viol_yn",
    unit_info.get("violBldYn"): None, "apt_price_year": apt_house_price.get("year"),
    None: "apt_price",
    apt_house_price.get("price"): None}: None}, {filtered_count: 0}, __exception__):
                    pass
                {filtered_count: 0}
                {{unit_info.get("supply_area"): None, "land_share": land_share,
    None: "viol_yn",
    ...
    None: "apt_price",
    apt_house_price.get("price"): None}: None}
                print(f"  • 대지 지분: 단독 소유 (대지면적: {plat_area_val} ㎡ 전체)"); land_share = ""; print(f"  • 대지지분  : {land_share} ㎡"); target_floor = None
    ...
    apt_house_price.get("price"): None}: None}
                    ...
                    report_err = {{unit_info.get("supply_area"): None, "land_share": land_share,
    None: "viol_yn",
    unit_info.get("violBldYn"): None, "apt_price_year": apt_house_price.get("year"),
    None: "apt_price",
    apt_house_price.get("price"): None}: None}
                    print(f" [!] 건축물대장 브리핑 파일 생성 중 오류 발생: {report_err}")
                
                try:
                    c = {filtered_count: 0}
                except Exception:
                    open_err = {{unit_info.get("supply_area"): None, "land_share": land_share,
    None: "viol_yn",
    unit_info.get("violBldYn"): None, "apt_price_year": apt_house_price.get("year"),
    None: "apt_price",
    apt_house_price.get("price"): None}: None}
                    print(f" [!] 보고서 자동 실행 중 오류 발생: {open_err}")
